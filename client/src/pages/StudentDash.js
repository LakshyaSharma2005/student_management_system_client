    ...
    const loadData = async () => {
      ...
      try {
        ...
        setAttendance([
          ...
          { date: '2025-12-03', subject: 'Operating Systems', status: 'Present' },
    ...
    <div style={styles.container}>
      ...
      <div style={styles.navbar}>
        <div style={styles.brand}>
          ...
          <h1>Student<span style={{color: '#f1c40f'}}>Portal</span></h1>
        </div>
      ...
      <div style={styles.mainGrid}>
        ...
        <div style={styles.sidebar}>
          ...
          <NavBtn label="🏅 Report Card" active={activeTab === 'marks'} onClick={() => setActiveTab('marks')} />
          <NavBtn label="💰 Fee Status" active={activeTab === 'fees'} onClick={() => setActiveTab('fees')} />
        </div>
        ...
        <div style={styles.content}>
            ...
            <>
              ...
              <div style={styles.welcomeCard}>
                ...
                <div style={styles.idCard}>
                  <small>ID CARD</small>
                  <h3>{student.name}</h3>
                  <p>{student.course} | {student.roll}</p>
                </div>
              </div>
            ...
            <div style={styles.card}>
              <div style={styles.cardHeader}>
                ...
                <div style={styles.progressBarBg}>
                  <div style={{...styles.progressBarFill, width: `${percentage}%`, background: percentage > 75 ? '#2ecc71' : '#e74c3c'}}></div>
                </div>
              ...
              <table style={styles.table}>
                <thead>
                  <tr style={styles.trHead}><th>Date</th><th>Subject</th><th>Status</th></tr>
                </thead>
                <tbody>
                  {attendance.map((r, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={styles.td}>{r.date}</td>
                      <td style={styles.td}><b>{r.subject}</b></td>
                      <td style={styles.td}><span style={r.status === 'Present' ? styles.statusGreen : styles.statusRed}>{r.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {activeTab === 'marks' && (
            <div style={styles.card}>
              ...
              <table style={styles.table}>
                <thead>
                  <tr style={styles.trHead}><th>Subject</th><th>Mid Term (50)</th><th>Final (100)</th><th>Total</th><th>Grade</th></tr>
                </thead>
                <tbody>
                  {marks.map((m, i) => (
                    <tr key={i} style={styles.tr}>
                      <td style={styles.td}><b>{m.subject}</b></td>
                      <td style={styles.td}>{m.mid}</td>
                      <td style={styles.td}>{m.final}</td>
                      <td style={styles.td}><strong>{m.mid + m.final}</strong>/150</td>
                      <td style={styles.td}><span style={styles.gradeBadge}>{m.grade}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const styles = {
  ...
  // Dashboard
  welcomeCard: { background: 'linear-gradient(135deg, #3498db, #2c3e50)', color: '#fff', padding: '30px', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', boxShadow: '0 4px 15px rgba(52, 152, 219, 0.3)' },
  welcomeText: { flex: 1 },
  idCard: { background: 'rgba(255,255,255,0.1)', padding: '15px', borderRadius: '8px', backdropFilter: 'blur(5px)', border: '1px solid rgba(255,255,255,0.2)', minWidth: '150px' },
  ...
  activityItem: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px', background: '#f8f9fa', borderRadius: '8px' },
  dateBadge: { background: '#e8f6fd', color: '#3498db', padding: '5px 10px', borderRadius: '6px', textAlign: 'center', fontWeight: 'bold' },
  ...
  gradeBadge: { background: '#34495e', color: '#fff', padding: '4px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: 'bold' },

  progressBarBg: { flex: 1, height: '8px', background: '#eee', borderRadius: '4px', margin: '0 20px' },
  progressBarFill: { height: '100%', borderRadius: '4px' },

  // Fee Card
  feeCard: { background: '#fff', padding: '40px', borderRadius: '12px', maxWidth: '600px', margin: '0 auto', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' },
  feeHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px', borderBottom: '1px solid #eee', paddingBottom: '20px' },
  paidStamp: { border: '2px solid #27ae60', color: '#27ae60', padding: '10px 20px', borderRadius: '8px', fontWeight: '900', letterSpacing: '2px', transform: 'rotate(-5deg)' },
